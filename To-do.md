...
- [ ] when i choose to flash a device, for everything except s3, the firmware
  isn't pre-selected.
...
- [ ] We need more states for paused, connecting, streaming.
- [ ] Need a way to change between wifi ap's from the device itself. choosing
  between existing presets is best.
- [ ] The list needs to be alphabetically sorted, and ideally would have
