...
enum class TouchController : uint8_t { None, Axs5106l, Cst9217, Cst816 };
...
struct Config {
  ...
  bool hasTouch() const {
    return touch != TouchController::None &&
           (pinTouchRst != NO_PIN || touchResetExio != 0) &&
           pinTouchInt != NO_PIN;
  }
...
/// Stable firmware-image target, distinct from the IDF chip token.
