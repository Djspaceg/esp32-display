// Reading the AXP2101 power-management IC on the ESP32-S3-Touch-AMOLED-1.75C:
// is a battery attached, is it charging, how full is it, what is its voltage.
//
// Every entry point is a no-op unless the board table says this board has a
// PMU (board::Config::hasBattery()), so the C6 binary compiles this file and
// then never talks to a chip that is not there - the same shape as
// board_touch.h returning false on the non-touch board.
//
...
// firmware needs five registers, read-only apart from two enable bits. The C6
// build is also already at 85% of its app partition and has to keep room for
// OTA, and while the C6 never executes any of this, keeping the dependency
// surface small is the same instinct board_touch.h documents for the AXS5106L.
// So: a minimal reader, not a vendored driver.
//
...
/// Bring up the PMU. Returns false when the board has no PMU, which is not an
/// error - it is a C6 answering honestly, exactly as boardtouch::init does on
/// the non-touch board - and false when the chip is there in the table but did
/// not answer. The caller uses the return value to decide whether to advertise
/// CAP_BATTERY, so a dead PMU means no advertised battery rather than a promise
/// of readings that never arrive.
///
...
inline bool init(const board::Config &cfg, bool verbose = true) {
  enabled = false;
  if (!cfg.hasBattery()) {
    if (verbose) Serial.printf("power: no PMU on %s\n", cfg.name);
    return false;
  }
...
/// Sample the PMU. Returns false when there is no PMU or a transaction failed,
/// so a caller never reports a half-populated reading as fact.
