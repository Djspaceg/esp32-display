...
esp_err_t esp_lcd_new_panel_gc9107(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    {
    case 16: // RGB565
        gc9107->colmod_val = 0x05;
        gc9107->fb_bits_per_pixel = 16;
        break;
    case 18: // RGB666
        gc9107->colmod_val = 0x06;
...
// A small, GC9107-specific default. The panel this project drives passes
// Waveshare's own sequence through vendor_config->init_cmds, so this default is
// only a fallback (init_cmds == NULL) and covers the register unlock, RGB565
// pixel format, inversion, and display-on that any GC9107 needs.
...
static esp_err_t panel_gc9107_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){gc9107->madctl_val}, 1), TAG, "send command failed");
    return ESP_OK;
}
