...
- [ ] **Surface failures that currently only reach the log.**
...
- [ ] **Stop the offline send path from burning work.**
...
- [ ] **Separate persisted settings from live telemetry in `panels.json`.**
...
- [ ] **Derive the mDNS `caps` TXT record from `DEVICE_CAPABILITIES`.**
...
- [ ] **Persist the per-panel capture source.**
...
- [ ] **Expose fps, pacing, and port in the UI.**
...
- [ ] **Extract a `SenderCore` library target.**
  ...
  - For reference, the protocol layer is well covered already: 20 Swift tests
    plus 140 host-compiled firmware checks.

- [ ] **Consider host-testing firmware logic that currently lives in the `.ino`.**
